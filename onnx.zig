//! The ONNX Runtime C API, wrapped for Zig.
//!
...
//! an owned error object otherwise. Both of those are wrapped here once, so a
//! caller can write `session.run(...)` and get a Zig error back.
//!
//! This is the module the package exports; the `build.zig` beside it is what
//! compiles the runtime the module calls into. A consumer gets both from one
//! call -- in build.zig:
//!
//!     const ort = @import("onnxruntime").select(b, target, optimize);
//!     ort.addImport(mod);
//!
//! and in the program:
//!
...
//!
//! The interesting part is `Device`. The CPU provider is compiled into the
//! runtime, but an NVIDIA GPU or an Intel NPU is reachable only through a
//! provider the runtime dlopens as a separate library, at a path given to it.
//! Registering that library makes the device show up in the runtime's device
//! list, and a session is then pinned to one by handing its `OrtEpDevice` to
//! the session options.
...
/// Which execution provider library a build carries, and so which one `Env
/// .registerProvider` is being handed. The build side of this package picks it
/// -- see `Provider` in build.zig -- and they have to agree, which is why the
/// name the runtime registers it under lives here rather than at the call site.
///
/// They are not interchangeable past the point of loading: OpenVINO compiles
/// the graph for the device on the way in and wants somewhere to keep the
/// result, while the CUDA provider's kernels were compiled by nvcc when it was
/// built and it takes no options of that kind at all.
pub const ProviderKind = enum {
    openvino,
    cuda,

    /// What to register the library under. The runtime hands this back to the
    /// provider's `CreateEpFactories`, and uses it in its own diagnostics.
    pub fn registrationName(self: ProviderKind) [:0]const u8 {
        return switch (self) {
            .openvino => "OpenVINO",
            .cuda => "CUDA",
        };
    ...
    /// Whether the provider compiles the graph when a session opens it, and so
    /// has something worth caching between runs. See `Session.open`.
    pub fn cachesCompiledGraphs(self: ProviderKind) bool {
        return switch (self) {
            .openvino => true,
            .cuda => false,
        };
...
/// the other two only exist once a provider library has been registered.
