...
bool IsInputSupported(const Node& node, const NodeArg& input, const OpBuilderInputParams& input_params,
  ...
  // Limit input shape rank to 5 for legacy NeuralNetwork format.
  // MLProgram / MIL supports higher rank tensors (rank >= 6).
  if (!input_params.create_mlprogram && shape.size() > 5) {
    LOGS(logger, VERBOSE) << "CoreML EP doesn't allow input shapes with rank greater than 5 in NeuralNetwork format. Input: "
                          << input_name << ", shape: " << Shape2String(shape);
    return false;
  }
