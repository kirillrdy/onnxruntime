...
std::string_view ModelBuilder::AddConstantImpl(std::string_view op_type, std::string_view value_type,
                                               ...
                                               std::optional<gsl::span<const int64_t>> shape) {
  MILSpec::Value input_value;
  if (weights_file_writer_ && value.size() >= 10) {
    auto& value_type_proto = *input_value.mutable_type();
    auto& tensor_type = *value_type_proto.mutable_tensortype();
    ...
    if (shape) {
      ...
      tensor_type.add_dimensions()->mutable_constant()->set_size(value.size());
    }
    MILBlob::Util::Span<const float> data(value.data(), value.size());
    uint64_t offset = weights_file_writer_->WriteData(data);
    auto* file_value = input_value.mutable_blobfilevalue();
    file_value->set_filename("@model_path/weights/weight.bin");
    file_value->set_offset(offset);
  } else {
    input_value = CreateTensorValue<float>(value, shape);
  }
...
std::string_view ModelBuilder::AddConstantImpl(std::string_view op_type, std::string_view value_type,
  ...
  if (weights_file_writer_ && value.size() >= 10) {
    auto& value_type_proto = *input_value.mutable_type();
    auto& tensor_type = *value_type_proto.mutable_tensortype();
    tensor_type.set_datatype(MILSpec::DataType::FLOAT16);
    if (shape) {
      tensor_type.set_rank(shape->size());
      for (const auto& dim : *shape) {
        tensor_type.add_dimensions()->mutable_constant()->set_size(dim);
    ...
    MILBlob::Util::Span<const MILBlob::Fp16> data(reinterpret_cast<const MILBlob::Fp16*>(value.data()), value.size());
    uint64_t offset = weights_file_writer_->WriteData(data);
    auto* file_value = input_value.mutable_blobfilevalue();
    file_value->set_filename("@model_path/weights/weight.bin");
    file_value->set_offset(offset);
  } else {
    input_value = CreateTensorValue<MLFloat16>(value, shape);
  }
  return AddTensorValueAsConstantOperation(op_type, value_type, std::move(input_value));
}
