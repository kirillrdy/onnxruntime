...
Status ReshapeOpBuilder::AddToModelBuilderImpl(ModelBuilder& model_builder,
  ...
  if (model_builder.CreateMLProgram()) {
    ...
    if (new_shape.size() == 6) {
      // Lower 6D shape (s0, s1, s2, s3, s4, s5) to 5D (s0*s1, s2, s3, s4, s5)
      // to conform with CoreML MIL reshape operator rank <= 5 limit.
      TensorShapeVector shape_5d = {
      ...
      AddOperationInput(*reshape_op, "shape",
                        model_builder.AddConstant(reshape_op->type(), "shape", ToConstSpan(shape_5d)));

      const auto dtype = node.OutputDefs()[0]->TypeAsProto()->tensor_type().elem_type();
      AddIntermediateOperationOutput(*reshape_op, node.OutputDefs()[0]->Name(), dtype, shape_5d);
...
bool ReshapeOpBuilder::IsOpSupportedImpl(const Node& node,
  ...
  if (new_shape.size() > 5) {
    if (!input_params.create_mlprogram) {
      LOGS(logger, VERBOSE) << "Reshape does not support new shape with rank greater than 5 in NeuralNetwork format. Input shape: "
                            << Shape2String(input_shape) << ", new shape: " << Shape2String(new_shape);
      return false;
    }
    if (new_shape.size() > 6) {
      LOGS(logger, VERBOSE) << "Reshape does not support new shape with rank greater than 6. Input shape: "
                            << Shape2String(input_shape) << ", new shape: " << Shape2String(new_shape);
      return false;
    }
  }
