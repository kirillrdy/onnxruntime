...
fn unpack(gpa: std.mem.Allocator, io: std.Io, opts: Options, name: []const u8) !void {
    ...
    const sha = opts.sha256 orelse "ready";
    const stamp = try std.fmt.bufPrint(&stamp_buf, "{s}/.extracted_{s}", .{ dest, sha });

    if (fileExists(io, stamp)) return;
    ...
    const tar_result = std.process.run(gpa, io, .{
        .argv = &.{ "tar", "-xf", opts.out, "-C", dest, "--strip-components=1" },
    }) catch null;
    ...
    var stamp_file = try cwd.createFile(io, stamp, .{});
    stamp_file.close(io);
...
fn downloadWithCurl(gpa: std.mem.Allocator, io: std.Io, opts: Options, dest_path: []const u8) !void {
    const result = try std.process.run(gpa, io, .{
        .argv = &.{ "curl", "-fsSL", "--retry", "5", "-o", dest_path, opts.url },
    });
    defer {
